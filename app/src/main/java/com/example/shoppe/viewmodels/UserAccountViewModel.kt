...
import com.example.shoppe.ShoppeApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    private fun getUser() {
        ...
        firestore.collection("user").document(auth.uid!!).get().addOnSuccessListener { document ->
        ...
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<ShoppeApplication>().contentResolver,
                    imageUri
                )
                ...
                saveUserInformation(user = user.copy(imagePath = imageUrl), false)
            } catch (e: Exception) {
                viewModelScope.launch {
    ...
    companion object {
        const val TAG = "UserAccountViewModel"
    }
}